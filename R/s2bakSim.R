...
#'
#' @description This function simulates the data needed for fitting SDMs for multiple species including generated trait and environments. The function is to primarily to illustrate the data structure, as well as demonstration of the provided functions.
#'
...
#' @param minimum.sightings numeric. The function will re-generate a new species until the minimum number of sightings (and survey presences) exceeds the provided value.
...
s2bakSim <- function(species = 30, sites = 3000, surv = 300,
  ...
  if (length(env.prob) != 2 | length(trait.prob) != 2) {
    stop("Invalid range for probabilities. Ensure that two numeric values are provided.")
  }
  if (surv > sites) {
    stop("Number of survey sites exceeds total number of sites: surv must be less than sites")
  }
  ...
  for (i in 1:species) {
    ...
    while (go && count <= max.iter) {
      ...
      p_e <- dnorm(dst(envs[[1]] - m[1], envs[[2]] - m[2]), sd = sdv) / dnorm(0, sd = sdv)
      ...
      # Probability of presence-only sighting
      # This is to generate sightings
      p_po <- p_d_t * p_d_e * p_e
