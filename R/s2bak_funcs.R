...
s2bak.S2 = function(formula, data, obs, surv = NA, background = NA,
  ...
  if(verbose){
    cat("Fitting", ifelse(all(is.na(surv)), "sightings-only", "S2"),"model for", length(specieslist), "species\n")
    cat(ncores, "cores\n")
  }

  # Add / if not there
  if(!is.na(readout)){
    readout = paste0(readout, ifelse(substr(readout,nchar(readout),nchar(readout))=="/","","/"))
  }
  ...
  if(typeof(formula) == "language"){
    ...
    if(!all(specieslist %in% names(formula))){
      stop("Incorrect formula type. Please provide a single formula or a list of formulas with each index named after the corresponding species.")
    }
  }else{
    stop("Incorrect formula type. Please provide a single formula or a list of formulas with each index named after the corresponding species.")
  }
  ...
  if(!is.na(readout)){
    fopts = paste0(readout, ifelse(all(is.na(surv)),"opts.SO.rds","opts.S2.rds"))
    saveRDS(l, fopts)
  ...
  l$sdm <- foreach(i = specieslist) %dopar% {
    if(verbose) cat("\t",i,"-",nrow(obs[obs$species==i,]),"occurrences\n")
    ...
    ind2 = c(obs[obs$species==i, which(colnames(obs) != "species")], background)
    # Row number
    if(is.na(ind)){
      tmp_dat = data[ind2,]
    }else{
  ...
  if(length(l$failure) > 0){
    warning(paste0("Models failed to fit for the following species: ", 
...
s2bak.SO = function(formula, data, obs, background = NA,
                    ...
                    ncores = 1, verbose = FALSE,
                    readout = NA, version = c("full", "short")[1], ...){
  return(s2bak.S2(formula, data, obs, surv = NA, background = background,
                    sdm.fun = sdm.fun, 
...
s2bak.BaK = function(predictions, surv, data, trait, verbose = FALSE){
  ...
  if(length(wh)>0){
    warning(paste("Columns from survey data missing from trait data and not excluded from the fitting:",colnames(surv)[wh]))
    surv= surv[,-wh]
  }
  specieslist = colnames(surv)

  # Model outputs
  out = list(species = specieslist,
              fullspecies = unique(trait$species))
  class(out) = "s2bak.BaK"

  if(verbose) cat("Fitting bias adjustment kernel on", length(specieslist), "survey species and",length(out$fullspecies),"species total.\n")

  # Throw error if predictions, surv and data are not aligned
  if(length(unique(c(nrow(predictions), nrow(data), nrow(surv)))) > 1){
    stop("Differing rows for predictions, survey and environment data. Sites/rows must correspond with each other.")
  }
  ...
  # Can replace with more complex models, if desired (but right now it's GLM with this)
  ...
  fit_adj = lapply(specieslist, FUN = function(x){
      ddf = data.frame(loc = 1:nrow(data), species = x, pa = 0, 
...
s2bak.S2BaK = function(formula, data, obs, surv, trait,
  ...
  ## First, fit SO model
  out$s2bak.SO = s2bak.SO(formula = formula, data = data, obs = obs, background = background, sdm.fun = sdm.fun, overlapbackground = overlapbackground, nbackground = nbackground, ncores = ncores, verbose = verbose, readout = readout, version = version,...)

  ## Next, fit S2 model
  ## Only for species with survey data
  if(all(is.na(surv))) stop("Missing survey data.")

  out$s2bak.S2 = s2bak.S2(formula = formula, data = data, obs = obs[obs$species %in% colnames(surv),], surv = surv, background = background, sdm.fun = sdm.fun, overlapbackground = overlapbackground, nbackground = nbackground, surv.formula = surv.formula, ncores = ncores, verbose = verbose, readout = readout, version = version, ...)
  ...
  if(is.na(ind)){
    ...
    surv_dat = data[match(surv[,ind], data[,ind]), -which(colnames(data)==ind)]
    surv2 = surv[,-which(colnames(surv)==ind)]
  }

  # Make predictions using out$SO (always type = "response")
  predictions = s2bak.predict.SOS2(out$s2bak.SO, surv_dat, doReadout = !is.na(readout), ncores = ncores, verbose = verbose, type = "response")
...
s2bak.predict.BaK = function(predictions, bak, trait, data){
  ...
  predictions2$zso_only = s2bak.truncate(as.vector(as.matrix(-log((1-predictions2$pred)/predictions2$pred))),-15,15)
...
s2bak.predict.SOS2 = function(model, newdata, predict.fun = predict.gam, doReadout = FALSE, verbose = FALSE, ncores = 1, ...){
  ...
  tmp[] = foreach(i = specieslist) %dopar% {
    ...
    if(dir){
      # Read in appropriate model, whose filename should be found in l$options$readout
  ...
  # Check failures
  failed = apply(tmp, 2, FUN = function(x){all(is.na(x))})
  if(any(failed)){
    warning(paste("Failed to predict for the following species:",
...
s2bak.predict = function(model, newdata, trait = NA, predict.fun = predict.gam, verbose = FALSE, ncores = 1, doReadout = FALSE, ...){
  ...
  if(class(model) == "s2bak.SO" | class(model) == "s2bak.S2"){
    return(s2bak.predict.SOS2(model, newdata, predict.fun, doReadout = doReadout, verbose = verbose, ncores = ncores, ...))
  }else if(class(model) == "s2bak.S2BaK"){
    if(is.null(model$s2bak.SO) | is.null(model$s2bak.BaK)) stop("Missing SO or BaK model(s).")
    ...
    if(!is.null(model$s2bak.S2)){
      ...
      out.S2 = s2bak.predict.SOS2(model = model$s2bak.S2, newdata = newdata, predict.fun = predict.fun, doReadout = doReadout, verbose = verbose, ncores = ncores, ...)
    ...
    out.SO = s2bak.predict.SOS2(model = model$s2bak.SO, newdata = newdata, predict.fun = predict.fun, doReadout = doReadout, verbose = verbose, ncores = ncores, ...)
